...
def AQHI_reader(csv_file):
    ...
    df = pd.read_csv(csv_file, header=None, skiprows=8, na_values=['*'], parse_dates=[0])
    df.columns = ['Date', 'Hour', 'Central/Western','Southern', 'Eastern', 'Kwun Tong', 'Sham Shui Po', 'Kwai Chung', 'Tsuen Wan', 'Tseung Kwan O', 'Yuen Long', 'Tuen Mun', 'Tung Chung', 'Tai Po', 'Sha Tin','North', 'Tap Mun', 'Causeway Bay', 'Central', 'Mong Kok']
    ...
    mean_row = mean_row.reindex(df.columns, axis=1)
...
csv_files = [f for f in os.listdir('Raw_data/AQHI_csv') if f.endswith('.csv')]

# AQHI_reader('Raw_data/AQHI_csv/202101_Eng.csv')
# loop through each CSV file and call the AQHI_reader function
for csv_file in csv_files:
    AQHI_reader(f"Raw_data/AQHI_csv/{csv_file}")
